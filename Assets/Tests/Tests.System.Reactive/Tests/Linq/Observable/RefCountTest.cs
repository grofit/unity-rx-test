...
{
    public partial class RefCountTest : ReactiveTest
    {
        ...
        public void RefCount_OnError()
        {
            ...
            res.Subscribe(_ => { Assert.True(false); }, ex_ => { Assert.AreSame(ex, ex_); }, () => { Assert.True(false); });
            res.Subscribe(_ => { Assert.True(false); }, ex_ => { Assert.AreSame(ex, ex_); }, () => { Assert.True(false); });
        }
        ...
        public void LazyRefCount_OnError()
        {
            ...
            res.Subscribe(_ => throw new Exception(), ex_ => { Assert.AreSame(ex, ex_); }, () => throw new Exception());
            res.Subscribe(_ => throw new Exception(), ex_ => { Assert.AreSame(ex, ex_); }, () => throw new Exception());
        }
